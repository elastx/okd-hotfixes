...
cp /var/run/secrets/kubernetes.io/serviceaccount/ca.crt /opt/hawkular/auth/ca.crt.tmp
grep -v ^# /opt/hawkular/auth/ca.crt.tmp >> /opt/hawkular/auth/ca.crt.fixed
csplit -z -f kubernetes-cas-to-import /opt/hawkular/auth/ca.crt.fixed '/-----BEGIN CERTIFICATE-----/' '{*}' > /dev/null
if [ $? != 0 ]; then
